...
def doDrive(filename):
    creds=doAuth()
    try:
	service = googleapiclient.discovery.build('drive', 'v2', credentials=creds)
...
def doGAS(mode,stringtogas):
	    ...
	    if(mode==1):
            	...
            	response = service1.scripts().run(scriptId='AKfycbxqaVxEPny9Rd3ltcMFD4AHVfExhvnuvUKnsGZq6hYzZXfFLXMelaoJW5wNKWMzKYV3',body=request).execute()
            	return response
	    elif(mode==2):
